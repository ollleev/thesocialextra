...
**Limite des reprises tardives :** l’API publique ne reçoit pas la date de fin de l’événement. Si le premier envoi n’a jamais été enregistré, une reprise incertaine peut créer une nouvelle annonce après cette fin, avec la durée initialement choisie. La même clé reste nécessaire pour éviter un doublon si le premier envoi avait réussi ; ne pas la remplacer pour contourner cette incertitude. Vérifier les annonces existantes et retirer une annonce devenue obsolète. Une échéance immuable contrôlée côté serveur reste à ajouter avant de garantir la fraîcheur de ce parcours.
